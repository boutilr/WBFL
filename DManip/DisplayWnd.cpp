...
IMPLEMENT_DYNCREATE(CDisplayWnd, CWnd)
...
std::shared_ptr<WBFL::DManip::iDisplayMgr> CDisplayWnd::GetDisplayMgr()
{
   return m_pDispMgr;
}

std::shared_ptr<const WBFL::DManip::iDisplayMgr> CDisplayWnd::GetDisplayMgr() const
{
   return m_pDispMgr;
}
...
{
   INT_PTR retval = CWnd::OnToolHitTest(point,pTI);
   if ( retval < 0 )
   {
      ...
      CDisplayWnd* pMe = const_cast<CDisplayWnd*>(this);
...
CRect CDisplayWnd::GetViewRect() const
{
   CRect rView;
   if ( m_IsPrinting )
...
#ifdef _DEBUG
void CDisplayWnd::AssertValid() const
{
	CWnd::AssertValid();
}

void CDisplayWnd::Dump(CDumpContext& dc) const
{
	CWnd::Dump(dc);
}
#endif //_DEBUG
...
void CDisplayWnd::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
   // Convert the point to logical coordinates
   CPoint logPoint = point;
   CClientDC dc(this);
...
void CDisplayWnd::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
   // Convert the point to logical coordinates
   CPoint logPoint = point;
   CClientDC dc(this);
...
BOOL CDisplayWnd::OnMouseWheel(UINT nFlags, short zDelta, CPoint point)
{
   // Convert the point to logical coordinates
   CPoint logPoint = point;
   CClientDC dc(this);
...
void CDisplayWnd::OnContextMenu(CWnd* pWnd,CPoint screenPoint)
{
   if ( !m_pDispMgr->OnContextMenu(pWnd,screenPoint) )
      HandleContextMenu(pWnd,screenPoint);

	CWnd::OnContextMenu(pWnd, screenPoint);
}

BOOL CDisplayWnd::OnNeedToolTipText(UINT id,NMHDR* pNMHDR,LRESULT* pResult)
{
   return m_pDispMgr->OnNeedToolTipText(id,pNMHDR,pResult);
}

//DROPEFFECT CDisplayWnd::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point) 
//{
//   // Convert the point to logical coordinates
//   CPoint logPoint = point;
...
//void CDisplayWnd::OnDragLeave() 
//{
//   m_pDispMgr->OnDragLeave();
//   CScrollView::OnDragLeave();
//}

//DROPEFFECT CDisplayWnd::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point) 
//{
//   // Convert the point to logical coordinates
//   CPoint logPoint = point;
//   CDManipClientDC dc(this);
...
DROPEFFECT CDisplayWnd::CanDrop(COleDataObject* pDataObject,DWORD dwKeyState,const WBFL::Geometry::Point2d& point)
{
   return DROPEFFECT_NONE;
}

void CDisplayWnd::OnDropped(COleDataObject* pDataObject,DROPEFFECT dropEffect, const WBFL::Geometry::Point2d& point)
{
   // Do nothing
}
...
void CDisplayWnd::ActivateZoomTask()
{
   auto task = m_pDispMgr->GetTaskFactory()->CreateZoomTask(this, nullptr, GREEN);
   m_pDispMgr->SetTask(task);
}

void CDisplayWnd::Zoom(CRect rect, bool reDraw)
{
   rect.NormalizeRect();
...
void CDisplayWnd::Zoom(const WBFL::Geometry::Rect2d& rect, bool reDraw)
{
   SetWorldViewRect(rect);
   CenterOnPoint(rect.Center(), reDraw);
}

void CDisplayWnd::Zoom(Float64 factor, bool reDraw)
{
   if (factor<=0.0)
   {
      ATLASSERT(false);
...
void CDisplayWnd::ScaleFont(LOGFONT& lfFont) const
{
...
void CDisplayWnd::CenterOnPoint(const WBFL::Geometry::Point2d& wPoint, bool reDraw)
{
...
void CDisplayWnd::CenterOnPoint(CPoint center, bool reDraw)
{
...
CRect CDisplayWnd::GetAdjustedLogicalViewRect()
{
...
WBFL::Geometry::Rect2d CDisplayWnd::GetAdjustedWorldViewRect()
{
...
void CDisplayWnd::SetMappingMode(WBFL::DManip::MapMode mm, bool reDraw)
{
...
WBFL::DManip::MapMode CDisplayWnd::GetMappingMode()
{
...
void CDisplayWnd::SetWorldViewRect(const WBFL::Geometry::Rect2d& rect)
{
...
//void CDisplayWnd::OnEndPrinting(CDC* pDC, CPrintInfo* pInfo) 
//{
...
void CDisplayWnd::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo,CRect rcDraw)
{
   m_IsPrinting = true;
...
//void CDisplayWnd::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo) 
//{
...
//   CenterOnPoint(wc);
//
//	CWnd::OnBeginPrinting(pDC, pInfo);
//}
...
void CDisplayWnd::HandleLButtonDown(UINT nFlags, CPoint logPoint)
{
   // Do nothing
}

void CDisplayWnd::HandleLButtonUp(UINT nFlags, CPoint logPoint)
{
   // Do nothing
}

void CDisplayWnd::HandleLButtonDblClk(UINT nFlags, CPoint logPoint)
{
   // Do nothing
}

void CDisplayWnd::HandleRButtonDown(UINT nFlags, CPoint logPoint)
{
   // Do nothing
}

void CDisplayWnd::HandleRButtonUp(UINT nFlags, CPoint logPoint)
{
   // Do nothing
}

void CDisplayWnd::HandleRButtonDblClk(UINT nFlags, CPoint logPoint)
{
   // Do nothing
}

void CDisplayWnd::HandleMouseMove(UINT nFlags, CPoint logPoint)
{
   // Do nothing
}

void CDisplayWnd::HandleMouseWheel(UINT nFlags, short zDelta, CPoint logPoint)
{
   // Do nothing
}

void CDisplayWnd::HandleKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
   // Do nothing
}

void CDisplayWnd::HandleContextMenu(CWnd* pWnd,CPoint logPoint)
{
   // Do nothing
}
